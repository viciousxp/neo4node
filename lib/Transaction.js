...
var Neo4j = require('./Neo4j')
  , db = new Neo4j('neo4node.sb01.stations.graphenedb.com', '24789', 'neo4node', 'QQMrtOg4GnP24LURqnbR');
...
function _parseResults(parse, results) {
    ...
    var parsedResults = {
        ...
        columns = results[0].columns.map(function (column) {
            return column;
        });
    ...
    for (var j = 0; j < results.length; j++) {
        results[j].data.map(function (row) {
            ...
            if (typeof row.rest !== 'undefined' && row.rest !== [null]) {
                var resultRow = {};
