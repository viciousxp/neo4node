...
function Neo4j(url, port, user, pass) {
    this.port = port || process.env.NEO4J_PORT || 7474;
    this.url = url || process.env.NEO4J_URL || 'localhost';
    this.user = user || process.env.NEO4J_USER || null;
    this.pass = pass || process.env.NEO4J_PASS || null;
};
...
Neo4j.prototype.REST = function (method, endPoint, data, callback) {
    ...
    */
    ...
    var options = {
        hostname: this.url,
        port: this.port,
        path: encodeURI('/db/data' + endPoint),
        method: method,
        headers: {
    ...
    if (this.user !== null && this.pass !== null) {
        var auth = 'Basic ' + new Buffer(this.user + ':' + this.pass).toString('base64');
