...
neo4node.prototype.getVersion = function(callback) {
...
neo4node.prototype.deleteNodeIndex = function (index, callback) {
    if (typeof index === 'undefined')
    ...
    db.REST('DELETE', '/index/node/' + index, function (err, status, body) {
        if (err) return callback(err);
...
neo4node.prototype.deleteRelationshipIndex = function (index, callback) {
    if (typeof index === 'undefined')
        throw new Error('missing callback');
    ...
    db.REST('DELETE', '/index/relationship/' + index, function (err, status, body) {
        if (err) return callback(err);
        if (status === 400) return callback('Index not found');
        if (status === 404) return callback('Index not found');
        if (status === 204) return callback(null);
